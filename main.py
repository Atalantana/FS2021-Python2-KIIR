...
def formular():
    ...
    if request.method == "POST":
        ...
        with open("data_plants.json", "r+") as outfile:
            ...
            data_plants["Timestamp"]=timestamp
...
def anzeigeAlle():
    with open("data_plants.json", "r+") as outfile:
        pflanzen = json.load(outfile)
    count = len(pflanzen)
    return render_template("AllePflanzen.html", anzeige=pflanzen, anzeigec=count)
...
def loeschen():
    with open("data_plants.json", "r+") as outfile:
        pflanzen = json.load(outfile)
    for i in range (len(pflanzen)):
        if pflanzen[i]==len(pflanzen):
            del pflanzen[i]
            break
